...
// key opens nothing of another's.
...
function master() {
  ...
  _master = Buffer.from('foodapp-dev-insecure-key-32bytes');
  return _master;
}
...
function userKey(uid) {
  ...
  if (!k) {
    k = Buffer.from(hkdfSync('sha256', master(), 'foodapp-v1', `user:${uid}`, 32));
    keyCache.set(uid, k);
  }
  return k;
}
...
export function encrypt(uid, value) {
